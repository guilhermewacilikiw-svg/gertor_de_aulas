  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg bg-neutral-900 border border-slate-800 rounded-3xl p-6 shadow-2xl text-white space-y-6">
        ...
        {success && (
          <div className="p-4 rounded-2xl bg-white/10 border border-white/20 text-white text-sm font-bold flex items-center justify-center gap-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {criteria.map((c) => (
              <div key={c.id} className="p-3.5 rounded-2xl bg-white/5 border border-white/5 space-y-2">
                <div className="flex items-center justify-between text-xs font-bold">
                  ...
                  <span className="text-white text-sm">{c.score.toFixed(1)}</span>
                </div>
                <input
                  type="range"
                  min="0"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-neutral-800 via-orange-500 to-red-900 hover:brightness-110 text-white font-black text-sm shadow-xl transition-all flex items-center justify-center gap-2 active:scale-98"
          >
            {loading ? (
