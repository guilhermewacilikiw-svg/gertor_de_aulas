...
export function LeadCaptureForm() {
  ...
  return (
    <div className="bg-gradient-to-br from-[#12121A] to-[#0a0a0f] border border-white/10 rounded-2xl p-8 sm:p-12 space-y-8 shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-2xl relative overflow-hidden">
      
      {/* Decorative Glows */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-red-600/10 rounded-full blur-[80px] -translate-y-1/2 translate-x-1/3"></div>
      <div className="absolute bottom-0 left-0 w-64 h-64 bg-red-500/10 rounded-full blur-[80px] translate-y-1/2 -translate-x-1/3"></div>
      ...
      {success && (
        <div className="relative z-10 p-5 rounded-2xl bg-red-500/10 border border-red-500/30 text-red-500 text-sm font-bold text-center flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(192,232,122,0.1)]">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full px-5 py-4 rounded-2xl bg-black/40 border border-white/10 text-white placeholder:text-gray-600 focus:outline-none focus:border-red-600 focus:ring-1 focus:ring-red-600 transition-all"
            />
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full px-5 py-4 rounded-2xl bg-black/40 border border-white/10 text-white focus:outline-none focus:border-red-600 focus:ring-1 focus:ring-red-600 transition-all appearance-none"
            >
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-5 px-8 rounded-2xl bg-gradient-to-r from-red-600 to-red-500 text-white font-black text-sm uppercase tracking-widest hover:brightness-110 transition-all flex items-center justify-center gap-3 mt-4 shadow-[0_10px_30px_rgba(125,122,232,0.3)] hover:shadow-[0_15px_40px_rgba(125,122,232,0.5)] hover:-translate-y-1"
        >
          {loading ? (
