...
import { Loader2, Mail, Lock, User, BookOpen, Phone, Calendar, FileText, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto pb-16 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center gap-4">
        <Link href="/escola/professores" className="p-3 bg-white/5 hover:bg-white/10 rounded-xl transition-colors text-white">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight">Novo Professor</h1>
          <p className="text-sm text-red-500 font-mono uppercase tracking-widest mt-1">INICIALIZAR ACESSO DOCENTE</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-white/80 ml-1 uppercase tracking-wider">Especialidade</label>
              <div className="relative">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  type="date"
                  name="birth_date"
                  className="w-full bg-black/50 border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white text-sm focus:outline-none focus:border-red-600 focus:ring-1 focus:ring-red-600 transition-all placeholder:text-white/30 [color-scheme:dark]"
                />
              </div>
            </div>
            ...
            <div className="space-y-2 md:col-span-2">
              <div className="p-4 bg-white/5 border border-white/10 rounded-2xl">
                ...
                <p className="text-xs text-gray-400">
                  O professor receberá a senha temporária <span className="text-red-400 font-mono font-black">senha123</span> e será obrigado a trocá-la no primeiro acesso.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="pt-8">
            <button 
              type="submit"
              disabled={loading}
              className="w-full py-4 bg-red-600 text-white font-black text-sm uppercase tracking-widest hover:brightness-110 transition-all shadow-lg shadow-red-600/20 flex items-center justify-center gap-2 rounded-2xl disabled:opacity-50"
            >
              {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : 'Cadastrar Professor'}
            </button>
          </div>
        </form>
      </div>
